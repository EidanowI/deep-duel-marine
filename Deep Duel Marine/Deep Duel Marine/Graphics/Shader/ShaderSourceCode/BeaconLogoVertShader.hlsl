...
VSOut main(float3 v_pos : Position, float3 v_normal : Normal, float2 v_UV : UV, float4 v_Color : Color)
{
    ...
    float3 wave_a = Wave(float3(-5.0f, 0.0f, 15.0f), float3(-0.2f, 0.0f, -0.7f), 2.5f, 0.03f, 0.3f);
    float3 wave_b = Wave(float3(-5.0f, 0.0f, 15.0f), float3(-1.0f, 0.0f, 0.0f), 3.0f, 0.04f, 0.3f);
    float3 wave_c = Wave(float3(-5.0f, 0.0f, 15.0f), float3(-0.5f, 0.0f, 0.5f), 1.8f, 0.02f, 0.4f);
    
    v_pos += (wave_a + wave_b + wave_c) * float3(0.15, 1.1f, 0.15f);
    
    ret.VS_Pos = mul(BUF0_projection, mul(BUF0_view, mul(BUF1_model, float4(v_pos.x, v_pos.y, v_pos.z, 1.0f))));
    ret.VS_Normal = v_normal;
    ret.VS_UV = v_UV;
    ret.VS_color = float4(BUF1_global_time.x, 0.0f, 0.0f, 0.0f);
